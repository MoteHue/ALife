...
def write_fitness(hosts, paras, gen, file=None):
    ...
    line1 = "{:.3f}, {:.3f}".format(best_host["objective"], best_para["objective"])
...
def simple_main():
    ...
    gens, best_host, best_para = do_the_ga(para_bias=[0.1, 0.9])
    ...
    with open("coev-out.dat",'w') as f:
        gens, best_host, best_para = do_the_ga(file=f, write_every=1, para_bias=[0.1, 0.9])
